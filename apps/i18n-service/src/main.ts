...
async function bootstrap() {
  ...
  const config = new DocumentBuilder()
    .setTitle('NovaAdmin I18n Service')
    .setDescription('国际化服务 API 文档')
    .setVersion('1.0')
    .addBearerAuth()
    .build();
  ...
  const serviceName = configService.get<string>('SERVICE_NAME', 'i18n-service');
  ...
  console.log(`🚀 I18n Service is running on: http://localhost:${port}`);
