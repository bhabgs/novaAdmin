...
// 根据用户权限过滤菜单
const filterMenusByPermissions = (menus: any[], permissions: string[]): any[] => {
  ...
  return menus.filter(menu => {
    ...
    ...menu,
    children: menu.children ? filterMenusByPermissions(menu.children, permissions) : [],
  }));
};
...
export const menuHandlers = [
  ...
  http.put('/api/menus/:id', async ({ params, request }) => {
    ...
    // 检查路径是否已被其他菜单使用
    if (body.path && flatMenus.some(m => m.path === body.path && m.id !== id)) {
      return HttpResponse.json(createErrorResponse('菜单路径已存在', 400));
    }
    ...
    return HttpResponse.json(createSuccessResponse(updatedMenu, '菜单更新成功'));
  }),
  ...
  http.delete('/api/menus/:id', async ({ params }) => {
    ...
    if (menu.parentId) {
      ...
      if (parentMenu) {
        parentMenu.children = parentMenu.children.filter((child: any) => child.id !== id);
      }
    } else {
  ...
  http.post('/api/menus/batch-delete', async ({ request }) => {
    ...
    for (const id of ids) {
      ...
      if (menu && menu.children && menu.children.length > 0) {
        return HttpResponse.json(createErrorResponse(`菜单"${menu.name}"有子菜单，请先删除子菜单`, 400));
      }
    }
